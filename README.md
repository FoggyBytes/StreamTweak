...
<div align="center">
  <img width="960" height="540" alt="Immagine 2026-09-09 170725" src="https://github.com/user-attachments/assets/53e0b8b4-79ed-4dae-b6a8-6c32aba48a09" />
</div>
...
## ✨ What's New in 8.3.0 — "The Phantom Update"

- **Sessions that were never happening** — the Dashboard could show a live session while nothing was streaming, and it survived closing and reopening the app: only a reboot cleared it. StreamTweak recognised a session by looking for a handful of words anywhere in the streaming server's log, and ordinary lines contained them — a Playnite bridge announcing its own pipe, a display being restored after a stream, any line naming a client whose name happens to contain "Moonlight". In the log attached to [issue #9](https://github.com/FoggyBytes/StreamTweak/issues/9), 177 of 205 detected session starts were of that kind
- **Words are now read where the server writes them** — at the start of its own message, not anywhere in the line. The server's real lines are unprefixed; the noise always carries a prefix, and that one property separates them
- **A session ends even if the server never says so** — StreamTweak watches the network sockets the stream itself runs on and closes the session about a minute and a half after they go away. This mattered only for clients other than StreamLight, which reports in every second: with a phone, a TV or any other Moonlight client, a session whose end went unlogged stayed open until the app was restarted
- **Reopening the app no longer resurrects a finished session** — before trusting a start line left in the log, StreamTweak checks whether the server is actually holding those sockets
- **Sessions carry the server's own identifier, on Vibeshine and Vibepollo** — both forks name each session with a uuid, so the end of one can no longer be read as the end of another
- ⚠️ The check that was supposed to notice a session already in progress had never once worked: it looked for a TCP connection that a live stream does not hold. Measured across a real session, then replaced
